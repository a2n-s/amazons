...
void store_v_hash_both(uint64_t sig, int v, int depth){
	int ind = sig & HASH_MASK;
	int d = SIZE+1 - depth;
	if (hash_table[ind].d <= d){
/*		printf("Storage condition satisfied... ");*/
		hash_table[ind].d = (uint8_t) d;
		hash_table[ind].v_inf = (int8_t) v;
		hash_table[ind].v_sup = (int8_t) v;
		hash_table[ind].sig = sig;
/*		printf("stored both : %d, %d at %16lx\n", v, d, sig);*/
	}
}
...
/*		printf("Storage condition satisfied... ");*/
		if (hash_table[ind].sig != sig){
/*			printf("unknown signature - reset the line\n");*/
			hash_table[ind].d = (uint8_t) d;
		...
		if      (g <= alpha)             { 
/*			printf("alpha storage... stored v_sup : %d, %16lx\n", g, sig);*/
			hash_table[ind].v_sup = (int8_t)g; }
		else if (g >=  beta)             { 
/*			printf("beta storage... stored v_inf : %d, %16lx\n", g, sig);*/
			hash_table[ind].v_inf = (int8_t)g; }
		else if (g >  alpha && g < beta) { 
/*			printf("inside storage... stored v_inf & v_sup : %d, %16lx\n", g, sig);*/
			hash_table[ind].v_inf = (int8_t)g; hash_table[ind].v_sup = (int8_t)g; }
/*		else { printf("no storage\n"); }*/
	}
}
...
/*		printf("node at %16lx/%6lx already visited\n", sig, sig&HASH_MASK);*/
		if (v_inf == v_sup) { 
/*			printf("Exact cut\n"); */
			return v_inf; }
		if (v_inf >= beta)  { 
...
/*	else { printf("new node at %16lx/%6lx\n", sig, sig&HASH_MASK); }*/
	...
	if (end == 1){ 
/*		printf("End of game at %16lx/%6lx\n", sig, sig&HASH_MASK);*/
		store_v_hash_both(sig, -color, depth); 
		return -color; }
...
/*					printf("Maximizing %2d and %2d at depth %d\n", best_value, v, depth);*/
		...
		for (int q = 0; q < NB_QUEENS; q++){
			...
			for (int m = 0; m < moves_size; m++){
				...
				for (int ar = 0; ar < arrws_size; ar++){
					...
					// unshoot
					board[arrws[ar].i*WIDTH + arrws[ar].j] = EMPTY; //
/*					printf("Minimizing %2d and %2d at depth %d\n", best_value, v, depth);*/
